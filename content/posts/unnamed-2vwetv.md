...
4. **高效的数据结构（Efficient Data Structures）**
    ...
    - Redis 不是简单地用哈希表存所有东西。它为不同场景**精心选择了最合适的底层数据结构**，并在运行时根据数据大小和类型进行**编码优化**。
    - 例如：
...
5. **非阻塞式 I/O（Non-blocking I/O）**
    ...
    - Redis 将所有 socket 都设置为**非阻塞模式**（non-blocking）。
